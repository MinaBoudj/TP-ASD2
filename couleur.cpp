...
{
   if (nom == "noir")    return NOIR;
   if (nom == "rouge")   return ROUGE;
   if (nom == "vert")    return VERT;
   if (nom == "jaune")   return JAUNE;
   if (nom == "bleu")    return BLEU;
   if (nom == "magenta") return MAGENTA;
   if (nom == "cyan")    return CYAN;
   if (nom == "blanc")   return BLANC;
   return BLANC;
}
